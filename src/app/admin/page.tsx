...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    ...
    fetch(`${API_BASE}/admin/v1/stats`, { headers }).then(r => r.json()).then(setStats).catch(() => {});
    fetch(`${API_BASE}/admin/v1/users`, { headers }).then(r => r.json()).then(setUsers).catch(() => {});
    fetch(`${API_BASE}/admin/v1/shops`, { headers }).then(r => r.json()).then(setShops).catch(() => {});
  }, [router]);
  ...
  const handleChangePlan = async (userId: string, newPlan: string) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/admin/v1/users/${userId}/plan`, {
        method: "PATCH",
