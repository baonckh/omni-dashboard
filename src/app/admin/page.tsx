...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    ...
    fetch(`${API_BASE}/panel-api/stats`, { headers }).then(r => r.json()).then((d) => { if (d && typeof d === 'object') setStats(d); }).catch(() => {});
    fetch(`${API_BASE}/panel-api/users`, { headers }).then(r => r.json()).then((d) => { if (d && Array.isArray(d.users)) setUsers(d.users); }).catch(() => {});
    fetch(`${API_BASE}/panel-api/shops`, { headers }).then(r => r.json()).then((d) => { if (d && Array.isArray(d.shops)) setShops(d.shops); }).catch(() => {});
  }, [router]);
