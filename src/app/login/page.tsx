...
export default function LoginPage() {
  ...
  const handleGoogle = async () => {
    ...
    try {
      ...
      // 2. Exchange Firebase token → backend JWT
      ...
      // 3. Sign in with NextAuth using the backend JWT
      ...
      if (result?.error) { setError("Đăng nhập thất bại"); setGoogleLoading(false); return; }
      router.push("/overview");
    } catch { setError("Không thể kết nối đến Google"); setGoogleLoading(false); }
  };
    ...
    <div className="min-h-screen bg-[#0A0A0A] text-white flex items-center justify-center p-4">
      <div className="w-full max-w-sm">
        ...
        <button
          ...
          className="w-full flex items-center justify-center gap-3 py-3 rounded-xl text-sm font-medium border transition-colors mb-4 disabled:opacity-50 disabled:cursor-not-allowed"
          style={{ backgroundColor: "rgba(255,255,255,0.05)", borderColor: "rgba(255,255,255,0.1)", color: "#A1A1AA" }}
          onMouseOver={e => { e.currentTarget.style.backgroundColor = "rgba(255,255,255,0.08)"; e.currentTarget.style.borderColor = "rgba(255,255,255,0.2)"; e.currentTarget.style.color = "#FFFFFF"; }}
          onMouseOut={e => { e.currentTarget.style.backgroundColor = "rgba(255,255,255,0.05)"; e.currentTarget.style.borderColor = "rgba(255,255,255,0.1)"; e.currentTarget.style.color = "#A1A1AA"; }}
        >
        ...
        <p className="text-center text-sm text-zinc-500 mt-6">
          Chưa có tài khoản?{" "}
          <Link href="/register" className="text-blue-400 hover:text-blue-300 font-medium transition-colors">Đăng ký</Link>
        </p>
      </div>
    </div>
  );
